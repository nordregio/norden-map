...
gdf.rename(columns={"jan17": "population2017", "jan22": "population2022"}, inplace=True)

gdf_points = gdf[["country", "population2017", "population2022", "geometry"]]
...
gdf = gdf_updated[
    ["population2017", "population2022", "muncode", "country", "geometry"]
]
...
df["population"] = df.apply(
    lambda row: row[population_mapping.get(row["country"], None)], axis=1
)

df = df.loc[df["population"] != 0].sort_values(by="population", ascending=False)
